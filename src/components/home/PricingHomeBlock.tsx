...
export default function PricingHomeBlock() {
  const { state } = useData();
  const { content } = state;
  const pricingBlock = content?.pricingBlock || {
    subtitle: "Прозрачные условия",
    title: "Сколько стоят услуги?",
    text: "Стоимость услуг фиксируются прозрачно до начала работ. Итоговая сумма зависит от:",
    badges: [
      "Сложности объекта",
      "Количества классов",
      "Региона регистрации",
  ...
  return (
    <section  id="pricing" className="animate-on-scroll py-32 bg-[#FBF3DC] relative overflow-hidden rounded-[40px] mx-4 my-12 shadow-sm">
      {/* Ambient glowing backgrounds */}
      <div className="absolute top-1/2 left-0 -translate-y-1/2 w-[600px] h-[600px] bg-white rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-16 group">
          ...
          <div className="w-full lg:w-[55%] shrink-0">
             <div className="w-full bg-white aspect-[4/3] border border-[#E5E7EB] rounded-[32px] flex flex-col items-center justify-center text-[#6B7280] shadow-[0_20px_50px_rgba(0,0,0,0.05)] group-hover:shadow-[0_0_50px_rgba(59,130,246,0.15)] group-hover:border-[#3B82F6]/30 transition-all duration-700 relative overflow-hidden group/image">
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <div className="text-base mt-4 text-[#3B82F6] font-bold px-6 py-2.5 rounded-full shadow-sm border border-[#3B82F6]/20">Место для фото</div>
                </div>
                )}
             </div>
          </div>
          ...
          <div className="w-full lg:w-[45%] flex flex-col flex-1">
            <div className="inline-flex items-center space-x-2 text-[#C8A028] font-bold tracking-widest uppercase text-[13px] md:text-sm mb-8 bg-[#C8A028]/10 px-5 py-2.5 rounded-xl self-start border border-[#C8A028]/20 shadow-[0_0_20px_rgba(200,160,40,0.1)]">
               ...
               <span>{pricingBlock.subtitle}</span>
            </div>
            
            <h2 className="text-2xl sm:text-3xl lg:text-4xl whitespace-nowrap font-black text-[#1B3F7A] mb-6 tracking-tight leading-tight shrink-0">{pricingBlock.title}</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
               {pricingBlock.badges.map((badge, idx) => (
                 <div key={idx} className="bg-white flex items-center font-bold text-[#1B3F7A] px-5 py-4 rounded-xl border border-[#3B82F6]/60 shadow-[0_0_20px_rgba(59,130,246,0.25)] hover:shadow-[0_0_35px_rgba(59,130,246,0.6)] hover:border-[#3B82F6] transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group/card">
                   ...
                   <span className="relative z-10 text-sm xl:text-base">{badge}</span>
                 </div>
               ))}
            </div>
            ...
            <div className="flex justify-center w-full mt-2">
              <Link 
                to="/pricing"
                className="inline-flex items-center justify-center bg-[#C8A028] hover:bg-[#E8C050] text-white px-10 py-5 rounded-2xl font-bold text-lg transition-all duration-300 shadow-[0_4px_15px_rgba(200,160,40,0.3)] hover:shadow-[0_4px_25px_rgba(200,160,40,0.5)] hover:-translate-y-1 focus:outline-none w-full sm:w-auto group/btn relative overflow-hidden"
              >
                ...
                {pricingBlock.buttonText} <ArrowRight className="w-6 h-6 ml-3 group-hover/btn:translate-x-1.5 transition-transform" />
              </Link>
            </div>
          </div>
        </div>
