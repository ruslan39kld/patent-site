...
export default function Problems() {
  ...
  return (
    <section  id="tasks" className="animate-on-scroll py-24 bg-[#EEF3FB] relative overflow-hidden rounded-[40px] mx-4 my-12 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mb-12 animate-on-scroll stagger-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-5 animate-on-scroll stagger-2">
          {situations.map((sit) => (
             <div 
              key={sit.id}
              onClick={() => setSelectedTask(sit)}
              className={`bg-white group cursor-pointer rounded-2xl p-6 transition-all duration-300 flex flex-col justify-between relative overflow-hidden ${sit.spanClass}`}
              style={{
        ...
        {selectedTask && (
          <div className="space-y-6">
            ...
            <button
              ...
              className="mt-8 w-full py-4 bg-[#C8A028] text-white rounded-xl font-bold text-base transition-all hover:bg-[#E8C050] shadow-[0_4px_15px_rgba(200,160,40,0.3)] hover:shadow-[0_4px_25px_rgba(200,160,40,0.5)] inline-flex items-center justify-center gap-2 group border-none outline-none"
            >
              Обсудить задачу
