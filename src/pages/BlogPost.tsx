...
export default function BlogPost() {
  ...
  const relatedPosts = (() => {
    const same = state.blogPosts.filter(p => p.category === post.category && p.slug !== post.slug).slice(0, 2);
    if (same.length < 2) {
      const extra = state.blogPosts
        .filter(p => p.slug !== post.slug && !same.find(rp => rp.slug === p.slug))
    ...
    <div className="bg-white min-h-screen">
      <article className="pb-24 pt-8">
        <div className="max-w-[780px] mx-auto px-6">
          ...
          <PostContent contentHtml={post.content} />
          ...
          <div className="mt-16 pt-8 border-t border-[#E5E7EB]">
            ...
            <div className="bg-[#EEF3FB] rounded-2xl p-8 sm:p-10 text-center flex flex-col items-center border border-[#1B3F7A]/10 shadow-sm mb-16">
              ...
              <button
                type="button"
                onClick={() => window.dispatchEvent(new CustomEvent('open-ai-bot', {
                  detail: { initialMessage: `Здравствуйте! Вы читали статью «${post.title}». Какие вопросы у вас остались? Чем я могу помочь?` }
                }))}
