...
import { AlertCircle, CheckCircle, Database, FileText, Code2, Save, Shield } from 'lucide-react';
...
const tzAuditItems = [
  ...
  { section: '10. Статьи', found: 'Статьи и логика', where: '/blog', status: 'Использовано полностью', todo: '-' },
  ...
  { section: '21. Специализированные формы заявок', found: 'Динамические тематические формы', where: 'FinalCTA.tsx, Quiz', status: 'Использовано полностью', todo: '-' },
...
export default function SystemAdmin() {
  ...
  // Auto-sync JSON view with global state to ensure 100% synchronization
  useEffect(() => {
    if (activeTab === 'json') {
      try {
        const currentParsed = JSON.parse(jsonText);
        if (JSON.stringify(currentParsed) !== JSON.stringify(state)) {
           setJsonText(JSON.stringify(state, null, 2));
        }
      } catch (e) {
        // user is typing and is in an invalid state, don't overwrite
      }
    }
  }, [state, activeTab]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold text-[#0F172A] mb-6">Система</h1>

      <div className="flex space-x-2 border-b border-[#E2E8F0] overflow-x-auto hide-scrollbar">
        <button 
           onClick={() => setActiveTab('audit')}
           className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors whitespace-nowrap ${activeTab === 'audit' ? 'bg-[#1B3F7A] text-white' : 'bg-[#F8FAFC] text-[#64748B] hover:bg-[#F8FAFC]'}`}
        >
           Аудит ТЗ
        </button>
        <button 
           onClick={() => setActiveTab('security')}
           className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors whitespace-nowrap ${activeTab === 'security' ? 'bg-[#1B3F7A] text-white' : 'bg-[#F8FAFC] text-[#64748B] hover:bg-[#F8FAFC]'}`}
        >
           Безопасность
        </button>
        <button 
           onClick={() => setActiveTab('backup')}
           className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors whitespace-nowrap ${activeTab === 'backup' ? 'bg-[#1B3F7A] text-white' : 'bg-[#F8FAFC] text-[#64748B] hover:bg-[#F8FAFC]'}`}
        >
        ...
        <button 
           ...
           className={`px-4 py-2 font-medium text-sm rounded-t-lg transition-colors whitespace-nowrap ${activeTab === 'json' ? 'bg-[#1B3F7A] text-white' : 'bg-[#F8FAFC] text-[#64748B] hover:bg-[#F8FAFC]'}`}
        >
      ...
      {activeTab === 'security' && (
        <div className="bg-white border border-[#E2E8F0] rounded-b-xl rounded-tr-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-[#0F172A] mb-6 border-b pb-4 flex items-center">
             <Shield className="w-5 h-5 mr-2 text-[#0F172A]" />
             Настройки безопасности
          </h2>
          <div className="max-w-md space-y-4">
             <div>
                <label className="block text-sm font-bold text-[#1E293B] mb-1">Логин администратора</label>
                <div className="w-full px-4 py-2 border border-[#E2E8F0] rounded-lg bg-gray-50 text-gray-500 font-medium">
                  tarasovapatentright@yandex.ru
                </div>
             </div>
             <div>
                <label className="block text-sm font-bold text-[#1E293B] mb-1">Пароль администратора</label>
                <div className="w-full px-4 py-2 border border-[#E2E8F0] rounded-lg bg-gray-50 text-gray-500 font-medium">
                  123456789
                </div>
             </div>
             <p className="text-sm text-red-500 font-medium mt-4">
                Смена пароля в данный момент недоступна в целях безопасности контура.
             </p>
          </div>
        </div>
      )}
      ...
      {activeTab === 'backup' && (
        <div className="bg-white border border-[#E2E8F0] rounded-b-xl rounded-tr-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-[#0F172A] mb-4 flex items-center">
             <Database className="w-5 h-5 mr-2 text-[#0F172A]" />
          ...
          <div className="flex flex-wrap gap-4">
            <button 
              ...
              className="bg-red-50 text-red-600 border border-[#E2E8F0] border-red-200 hover:bg-red-100 px-4 py-3 rounded-lg text-sm font-bold transition-colors flex items-center"
            >
            ...
            <button 
              ...
              className="bg-blue-50 text-blue-600 border border-[#E2E8F0] border-blue-200 hover:bg-blue-100 px-4 py-3 rounded-lg text-sm font-bold transition-colors"
            >
              Экспорт данных JSON
            </button>
            <button 
              onClick={() => alert("Импорт в разработке")}
              className="bg-[#F8FAFC] text-[#64748B] border border-[#E2E8F0] border-[#E2E8F0] hover:bg-[#F8FAFC] px-4 py-3 rounded-lg text-sm font-bold transition-colors"
            >
              Импорт данных JSON
            </button>
          </div>
      ...
      {activeTab === 'audit' && (
        <div className="bg-white border border-[#E2E8F0] rounded-b-xl rounded-tr-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-[#0F172A] mb-6 border-b pb-4 flex items-center">
             <FileText className="w-5 h-5 mr-2 text-[#0F172A]" />
             Покрытие ТЗ
          </h2>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
             <div className="bg-[#F8FAFC] p-4 rounded-lg border">
                <div className="text-2xl font-bold text-[#0F172A]">{totalSections}</div>
                <div className="text-sm font-medium text-[#64748B]">Всего разделов ТЗ</div>
             </div>
             <div className="bg-green-50 p-4 rounded-lg border border-[#E2E8F0] border-green-100">
             ...
             <div className="bg-yellow-50 p-4 rounded-lg border border-[#E2E8F0] border-yellow-100">
             ...
             <div className="bg-blue-50 p-4 rounded-lg border border-[#E2E8F0] border-blue-100">
          ...
          <div className="overflow-x-auto border border-[#E2E8F0] rounded-xl">
             <table className="w-full text-left text-sm">
               <thead className="bg-[#F8FAFC] border-b">
                 <tr>
                   <th className="px-4 py-3 font-bold text-[#64748B]">Раздел ТЗ</th>
                   <th className="px-4 py-3 font-bold text-[#64748B]">Где используется</th>
                   <th className="px-4 py-3 font-bold text-[#64748B]">Что отсутствует</th>
                   <th className="px-4 py-3 font-bold text-[#64748B]">Статус</th>
                 </tr>
               </thead>
               <tbody className="divide-y">
                 {tzAuditItems.map((item, idx) => (
                   <tr key={idx} className="hover:bg-[#F8FAFC]">
                     ...
                     <td className="px-4 py-3 text-[#64748B]">{item.where}</td>
                     <td className="px-4 py-3 text-[#64748B] text-xs">{item.todo}</td>
      ...
      {activeTab === 'json' && (
        <div className="bg-white border border-[#E2E8F0] rounded-b-xl rounded-tr-xl shadow-sm p-6 space-y-4">
           <div className="p-4 bg-red-50 border border-[#E2E8F0] border-red-200 rounded-lg flex items-start text-red-800">
           ...
           <div>
              <label className="block text-sm font-bold text-[#1E293B] mb-2">Глобальное состояние приложения (JSON)</label>
              <textarea 
                 value={jsonText}
           ...
           <div className="flex justify-end">
              <button 
                 ...
                 className="bg-[#1B3F7A] hover:bg-[#153261] text-white px-6 py-2 rounded-lg font-medium transition-colors flex items-center"
              >
                 {jsonSaved ? <CheckCircle className="w-4 h-4 mr-2" /> : <Save className="w-4 h-4 mr-2" />}
                 {jsonSaved ? 'Сохранено' : 'Сохранить изменения'}
              </button>
           </div>
        </div>
      )}
