...
export default function Login({ onLogin }: LoginProps) {
  const { state } = useData();
  const expectedLogin = state?.content?.adminLogin || 'ruslan39kld@neurbiz.ru';
  const expectedPassword = state?.content?.adminPassword || '123456789';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col justify-center items-center px-4 relative selection:bg-[#1B3F7A] selection:text-[#0F172A] font-sans text-[#1E293B]">
      <div className="absolute top-1/4 right-1/4 w-96 h-96 bg-[#1B3F7A]/10 rounded-full blur-[100px] z-0 pointer-events-none"></div>
      <div className="absolute bottom-1/4 left-1/4 w-96 h-96 bg-[#22D3EE]/10 rounded-full blur-[100px] z-0 pointer-events-none"></div>
      ...
      <div className="bg-white/80 backdrop-blur-xl border border-[#E2E8F0] p-8 rounded-2xl shadow-xl w-full max-w-md relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button 
            type="submit" 
            className="w-full bg-[#1B3F7A] hover:bg-[#1B3F7A]/90 text-[#0F172A] font-bold py-3.5 rounded-xl transition-all shadow-md mt-6"
          >
            Войти в систему
          </button>
        </form>
