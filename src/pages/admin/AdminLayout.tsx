...
import { LayoutDashboard, LogOut, Code, User, FileText, Briefcase, MessagesSquare, Settings, DollarSign, HelpCircle, Star, PenTool, CheckCircle2, AlertCircle, X, Bot } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] flex font-sans">
      ...
      <main className="flex-1 overflow-auto bg-[#F8FAFC]">
         <div className="md:hidden bg-white text-[#0F172A] p-4 flex justify-between items-center z-10 relative border-b border-[#E2E8F0]">
            <div className="font-bold">Admin Panel</div>
            <div className="flex gap-2">
               <Link to="/" className="text-sm border border-[#E2E8F0] px-3 py-1 rounded hover:bg-[#F1F5F9] transition-colors">Сайт</Link>
               <button onClick={handleLogout} className="text-sm border border-[#E2E8F0] text-red-400 px-3 py-1 rounded hover:bg-[#F1F5F9] transition-colors"><LogOut className="w-4 h-4" /></button>
            </div>
         </div>
         {/* Mobile menu (simple) */}
         <div className="md:hidden bg-white border-b border-[#E2E8F0] flex overflow-x-auto whitespace-nowrap p-2 shadow-sm relative z-0 hide-scrollbar">
            {links.map((link) => (
               <Link 
                 key={link.to}
                 to={link.to} 
